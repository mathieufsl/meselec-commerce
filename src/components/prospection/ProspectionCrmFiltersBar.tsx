...
export type ProspectionCrmFiltersBarProps = {
...
export function ProspectionCrmFiltersBar({
  variant,
  search,
  onSearchChange,
  filterDepartement,
      ...
      <div className={cn("lg:hidden", className)}>
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Rechercher commune, maire, email…"
              value={search}
  ...
  return (
    <div className={cn("flex flex-wrap items-center gap-2 rounded-lg border bg-muted/30 p-2", className)}>
      <div className="relative w-full sm:w-[280px] lg:w-[320px] xl:w-[360px]">
        <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <Input
          placeholder="Rechercher commune, maire, email, cible…"
          value={search}
          onChange={(e) => onSearchChange(e.target.value)}
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" className="max-h-[min(60vh,360px)] w-72 overflow-y-auto">
          <DropdownMenuItem onClick={() => onFilterAggloChange("")}>
            {!filterAgglo && "✓ "}Toutes agglomérations
          </DropdownMenuItem>
          {agglos.map((agglo) => (
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" className="w-40">
          <DropdownMenuItem onClick={() => onFilterNuanceChange("")}>
            {!filterNuance && "✓ "}Toutes nuances
          </DropdownMenuItem>
          {nuances.map((nuance) => (
            <DropdownMenuItem key={nuance} onClick={() => onFilterNuanceChange(nuance)}>
              {filterNuance === nuance && "✓ "}
              {nuance}
            </DropdownMenuItem>
          ))}
